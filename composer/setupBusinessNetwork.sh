...
# sleep 3
# composer participant add -c patientadmin@patient-network -d '{"$class":"org.example.patientnewtork.Patient","id":"patient1", "name":"Joe","mobilePhone":"12344","city":"shanghai"}'
# # add on doctor participant
# sleep 3
# composer participant add -c doctoradmin@patient-network -d '{"$class":"org.example.patientnewtork.Doctor","id":"doctor1", "hospital":"10th hospital","name":"Alice","mobilePhone":"6743","city":"shanghai"}'

composer transaction submit --card patientadmin@patient-network -d '{"$class":"org.example.patientnewtork.SetupDemo"}'

# create identities for the business work
composer card delete -c patient@patient-network
sleep 3
composer identity issue -c patientadmin@patient-network -f patient.card -u patient -a "resource:org.example.patientnewtork.Patient#Patient_1"
sleep 2
composer card import -f patient.card

composer card delete -c radiologist@patient-network
sleep 3
composer identity issue -c doctoradmin@patient-network -f radiologist.card -u radiologist -a "resource:org.example.patientnewtork.Doctor#Doctor_1"
sleep 2
